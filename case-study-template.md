...
```
 ...
 a в stack_prof я увидел что метод `add_user_stat` потребляет ~45% памяти
 и обноруженная строка как раз в этом методе
 я снова посмотртел на отчет memory_profiler и увидел,
 что следующие по потреблению строки в том же методе
 и они не сильно отличаются от топовой по потреблению
 буду оптимизировать по одной строке с прогоном тестов и бенчмарка (но без профайлера)
 и в конце посмотрю как изменился отчет профайлера
## Результаты
