...
также как и врошлый раз я добавил файлик `bin/feetback_loop`
...
### Инструменты
Для того, чтобы найти "точки роста" для оптимизации я воспользовался memory-profiler, stackprof, ruby-profiler
...
- добавил frozen_string_literal: true
  это экономит ~ 145mb аллокаций

## Результаты
В результате проделанной оптимизации наконец удалось обработать файл с данными.
Удалось улучшить метрику системы до константных 30мб и уложиться в заданный бюджет 70мб.
также программа заметно ускорилась ~20s для всего датасета это обгоняет мою программу из прошлого задания в 2 раза

memory-profiler оказался очень удобным (можно увидеть топ мест по потреблению памяти)
но ruby-prof и stack-prof работают быстрее
и позволяют понять какие методы, а не конкретные строки, сколько аллоцируют памяти

память тяжело дебажить
не хватает инструмента который показывает где накапливаются объекты
(в каких местах расбухает программа)
понял что много аллокаций != расбухание
понял что много вызовов GC существенно замедляет программу

valgrind massif - крутая тула)

## Защита от регрессии производительности
Для защиты от потери достигнутого прогресса при дальнейших изменениях программы я написал тест
```ruby
