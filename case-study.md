...
Так же возникла идея использовать конструкцию вида
...
## RubyProf::MEMORY - top: slice, parse_session, parse_user
RubyProf в режиме MEMORY показывает то же самое, больше всего памяти тратит slice,
за ним следуют parse_session и parse_user, оба в пределах 2%. Попробовал использовать вместо slice
slice!, но мерика ухудшилась и я откатил изменения. 

## Valgrind massif visualier
![valgrind massif visualier](./2019-09-02.png)
На скриншоте видно, что потребление памяти растет относительно равномерно, а затем в конце возникает пик.
Судя по всему, мы видем как данные накапливаются в report и затем в конце генерируется общий json.

Судя по всему, не получится уложиться в бюджет, удерживая весь report в памяти, и необходимо
провести рефакторинг таким образом, чтобы  отдельные части отчета генерировались и сохранядись во
временный файл, а затем происходила общая сборка отчета

Я вернул обработку строк через split, так как выигрышь от побуквенного разбора оказался очень 
небольшим, при этом сильно ухудшив читаемость скрипта, так же вернул # frozen_string_literal: true

Рассмотрев data.txt файл, я обратил внимание, что пользователи и сессии отсортированы по id пользователя.
Таким образом, если очередная строка файла содаржит пользователя - то статистика предыдущего
пользователя фактически обсчитана, и может быть сохранена во временный файл для освобождения памияти.

Значение метрики 182мб(файл размером 10 мб), попробуем внести изменения в код.

После рефакторинга метрика упала до 21 Mb, коммитим 

## Итоги

После проведения последнего рефакторинга, я замерил потребление памяти при обработке файла data_large.txt,
и оно составило 22 MB, что укладывается в бюджет. Проверим, что потребление памяти укладывается в бюджет на всем
протяжении работы программы: 

![valgrind massif visualier](./2019-09-03.png)

На графике видна почти идеальная прямая, следовательно скрипт сможет обработать файл любого объема,
оставаясь в рамках заданного бюджета.

Успешность последнего шага оптимизации говорит о том, что с него следовало начать, возможно это позволило
бы раньше вписаться в бюджет и сберечь время.

Защитим достигнутый результат, дописав тест на используемую память. 
