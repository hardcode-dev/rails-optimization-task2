...
- какой отчёт показал главную точку роста
	...
	- Строка user.stats[:dates].sort!.reverse! находилась в методе перебора сессий, где вызывалась неимоверно много раз...
	- Наконец правильно установил переменную среды MALLOC_ARENA_MAX=2, но по памяти ничего не поменялось

- как вы решили её оптимизировать

	- Перенес строку user.stats[:dates].sort!.reverse! в другое место кода, где она выхывается всего лишь 3 раза (по кол-ву юзеров), сразу же затраченная память уменьшилась с 608 MB до 539 MB!

- как изменилась метрика

	- MEMORY USAGE: 539 MB (Изначально выделенная память была 997 MB, почти 1 GB)
	- Тест 'Memory test' для файла в 100_000 строк выделенная память составляет 32 MB

- как изменился отчёт профилировщика

## Результаты
В результате проделанной оптимизации не удалось обработать файл с данными data_large.txt.
Для его обработки раньше выделялось 2700 MB, на данный момент выделяется 740 MB памяти, что в 4 раза меньше!
К сожалению до 70 MB довести не удалось...( ...пока...

Удалось улучшить метрику системы с 300 MB (на файле с 100_000 строками) до 32 MB и уложиться в заданный бюджет.


## Защита от регрессии производительности
Для защиты от потери достигнутого прогресса при дальнейших изменениях программы был написан специальный тест Memory Test
