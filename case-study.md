...
### Не моя, а социалистическая находка №4

- По факту, то проблема в том, что я не записываю файл прямо во время цикла `file_lines.each`, а храню все хэши в памяти. Она была почти видна на предыдущем лупе, но сейчас выразилась еще более четко. Тут я пошел искать бест практики, так сказать. :) 
```
Allocated String Report
-----------------------------------
    423067  "session"
...
- Метрика на large_data.txt
```
MEMORY USAGE: 36 MB
Completed in 16.453 ms
```
- Так как мы уложились в наш бюджет, то заканчиваю цикл и перехожу к итогам.

![](https://i.imgur.com/Ttm5JSx.png)
## Результаты
В результате проделанной оптимизации наконец удалось обработать файл с данными.
Удалось улучшить метрику системы с первоначального убийства несчастного процессика, требующего всю память компьютера на себя и в конечном итоге умирающего смертью храбрых до `36 мб` с временем запуска `17 секунд`(ну и самое главное отсутствие раздувания памяти) и уложиться в заданный бюджет.
...
Для защиты от потери достигнутого прогресса при дальнейших изменениях программы было написано два теста, проверяющих общее время работы и количество выделяемой памяти в конце работы программы.
