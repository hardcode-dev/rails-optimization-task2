...
describe FastReportBuilder do
  before do
    # File.write('result.json', '')
    File.write('payloads/data.txt',
               'user,0,Leida,Cira,0
...
')
  ...
  describe '#call' do
    subject(:generate_report) do
      described_class.new.call(input_filename, 'result-rspec.json')
    end

    let(:input_filename) { 'data.txt' }
    ...
    let(:actual_result) do
      JSON.parse(File.read('result-rspec.json'))
    end
    ...
    context 'when using big file' do
      let(:input_filename) { 'data_6000.txt' }
