...
class BenchmarkTimeTest < Test::Unit::TestCase
  def test_result
    assert_equal Benchmark.realtime { work('data_100K.txt') } < 1, true
  end
end

class BenchmarkMemoryTest < Test::Unit::TestCase
  def test_result
    reader, writer = IO.pipe
    Process.fork do
      work('data_100K.txt')
      writer.puts(`ps -o rss= -p #{Process.pid}`.to_i / 1024)
    end
    Process.waitall
    assert_equal reader.gets.to_i < 60, true
  end
end
